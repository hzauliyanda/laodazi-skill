...
export function findChromeExecutable(): string | undefined {
  const override = process.env.MULTI_PLATFORM_BROWSER_CHROME_PATH?.trim();
...
export function getDefaultProfileDir(): string {
  ...
  return path.join(base, 'multi-platform-publish-profile');
}
...
export async function launchChrome(url: string, profileDir?: string): Promise<{ cdp: CdpConnection; chrome: ReturnType<typeof spawn> }> {
  ...
  if (!chromePath) throw new Error('Chrome not found. Set MULTI_PLATFORM_BROWSER_CHROME_PATH env var.');
